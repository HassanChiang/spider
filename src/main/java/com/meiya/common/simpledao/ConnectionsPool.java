...
public class ConnectionsPool {
	...
	private String user;
	private String pwd;
	private int initCon = 3;// 连接池中初始创建连接数
	private int maxCon = 10;// 连接池中允许创建的最大的连接数
	private int incCon = 3;// 请求连接数大于活动连接数时，允许请求用户创建的连接数(必须大于0)
	...
	private void initConfig() {
		this.driver = Config.getProps("driver");
		this.url = Config.getProps("url");
		this.user = Config.getProps("user");
		this.pwd = Config.getProps("pwd");

		if(Config.getIntegerProps("initCon")>0)
			this.initCon = Config.getIntegerProps("initCon");
		if(Config.getIntegerProps("maxCon")>0)
			this.maxCon = Config.getIntegerProps("maxCon");
		if(Config.getIntegerProps("incCon")>0)
			this.incCon = Config.getIntegerProps("incCon");
		if(Config.getIntegerProps("overtime")>0)
			this.overtime = Config.getIntegerProps("overtime");
		if(Config.getIntegerProps("sleeptime")>0)
			this.sleeptime = Config.getIntegerProps("sleeptime");
	}
